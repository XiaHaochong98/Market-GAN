...
def data_sample_worker(data, seq_len, features, history_length, full_data, tic, dynamics, args):
    # sort the data by 'date' to make sure the data is in order
    # the date is in the format of 'YYYY-MM-DD'
    data = data.sort_values(by='date')

    # return the sample of data splited to train and test
    time = []
    ...
    for interval_index, interval in enumerate(intervals):
        ...
        data_seg = data_seg.loc[:, ['date'] + features]
        ...
        for i in range(0, len(data_seg) - seq_len):
            ...
            sample_date = data_sample.loc[:, ['date']]
            history_date = history_sample.loc[:, ['date']]
            history_sample = history_sample.loc[:, features]

            # reparameterization the data_sample and history_sample
            ori_data_sample = data_sample.loc[:, features].copy()
            ...
            scaler = np.vstack(
                [low_scaler_vec, O_minus_L_scaler_vec, C_minus_L_scaler_vec, H_minus_maxOC_scaler_vec]).T  # dim: (n,4)
            time.append(seq_len)
            ...
            sample_date_vec.append(sample_date.iloc[0, :])
            history_date_vec.append(history_date.iloc[0, :])

    # split the all the variables into train and test with args.train_rate
    train_num = int(len(normalized_data_samples) * args.train_rate)
    normalized_data_samples_train = normalized_data_samples[:train_num]
    normalized_data_samples_test = normalized_data_samples[train_num:]
    normalized_history_samples_train = normalized_history_samples[:train_num]
    normalized_history_samples_test = normalized_history_samples[train_num:]
    original_data_samples_train = original_data_samples[:train_num]
    original_data_samples_test = original_data_samples[train_num:]
    original_history_samples_train = original_history_samples[:train_num]
    original_history_samples_test = original_history_samples[train_num:]
    time_train = time[:train_num]
    time_test = time[train_num:]
    scaler_train = scaler[:train_num]
    scaler_test = scaler[train_num:]
    last_hist_vec_train = last_hist_vec[:train_num]
    last_hist_vec_test = last_hist_vec[train_num:]
    sample_date_vec_train = sample_date_vec[:train_num]
    sample_date_vec_test = sample_date_vec[train_num:]
    history_date_vec_train = history_date_vec[:train_num]
    history_date_vec_test = history_date_vec[train_num:]

    # check range of 'date' in normalized_data_samples_train,normalized_data_samples_test,normalized_history_samples_train,normalized_history_samples_test
    # find the min and max date in normalized_data_samples_train,normalized_data_samples_test,normalized_history_samples_train,normalized_history_samples_test
    print(f'{tic} {dynamics} train range', np.min(sample_date_vec_train), np.max(sample_date_vec_train))
    print(f'{tic} {dynamics} test range', np.min(sample_date_vec_test), np.max(sample_date_vec_test))
    print(f'{tic} {dynamics} history train range', np.min(history_date_vec_train), np.max(history_date_vec_train))
    print(f'{tic} {dynamics} history test range', np.min(history_date_vec_test), np.max(history_date_vec_test))

    # # display the shape of the data and history
    print(f'{tic} {dynamics} train shape', np.array(normalized_data_samples_train).shape)
    print(f'{tic} {dynamics} test shape', np.array(normalized_data_samples_test).shape)
    print(f'{tic} {dynamics} history train shape', np.array(normalized_history_samples_train).shape)
    print(f'{tic} {dynamics} history test shape', np.array(normalized_history_samples_test).shape)
    print(f'{tic} {dynamics} train scaler shape', np.array(scaler_train).shape)
    print(f'{tic} {dynamics} test scaler shape', np.array(scaler_test).shape)
    print(f'{tic} {dynamics} train last_hist_vec shape', np.array(last_hist_vec_train).shape)
    print(f'{tic} {dynamics} test last_hist_vec shape', np.array(last_hist_vec_test).shape)
    # display the original data
    print(f'{tic} {dynamics} train original data shape', np.array(original_data_samples_train).shape)
    print(f'{tic} {dynamics} test original data shape', np.array(original_data_samples_test).shape)
    print(f'{tic} {dynamics} train original history shape', np.array(original_history_samples_train).shape)
    print(f'{tic} {dynamics} test original history shape', np.array(original_history_samples_test).shape)

    # pack the variables into four dictionaries, train/test, normalized/original
    original_data_sample_train = {'data': original_data_samples_train, 'time': time_train, 'scaler': scaler_train,
                                  'last_hist_vec': last_hist_vec_train, 'history': original_history_samples_train}
    original_data_sample_test = {'data': original_data_samples_test, 'time': time_test, 'scaler': scaler_test,
                                 'last_hist_vec': last_hist_vec_test, 'history': original_history_samples_test}
    normalized_data_sample_train = {'data': normalized_data_samples_train, 'time': time_train, 'scaler': scaler_train,
                                    'last_hist_vec': last_hist_vec_train, 'history': normalized_history_samples_train}
    normalized_data_sample_test = {'data': normalized_data_samples_test, 'time': time_test, 'scaler': scaler_test,
                                   'last_hist_vec': last_hist_vec_test, 'history': normalized_history_samples_test}

    return original_data_sample_train, original_data_sample_test, normalized_data_sample_train, normalized_data_sample_test
...
def conditional_rescale_data(ori_data, scaler, differential_features, last_hist_value, scaler_order,
    ...
    # reverse the reparameterization
