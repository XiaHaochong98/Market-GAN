...
from MDM.utils import get_attr, labeling_util as util, market_dynamics_modeling_analysis
from pathlib import Path
...
class Linear_Market_Dynamics_Model(Market_dynamics_model):
    ...
    def run(self):
        ...
        if self.OE_BTC == True:
            ...
            raw_data.to_csv(process_data_path)
            self.data_path = process_data_path
        worker = util.Worker(data_path=self.data_path, method='slice_and_merge', filter_strength=self.filter_strength,
                             key_indicator=self.key_indicator,
                             timestamp=self.timestamp, tic=self.tic, labeling_method=self.labeling_method,
        ...
        if self.PM:
            ...
            process_datafile_path = os.path.join(output_path,
                                                 file_name + '_labeled_' + self.model_id + '.' + extension).replace(
                "\\", "/")
            if extension == 'csv':
                merged_data.to_csv(process_datafile_path, index=False)
        ...
        market_dynamic_modeling_visualization_paths = worker.plot(worker.tics, self.slope_interval, output_path,
                                                                  self.model_id)
        print('plotting done')
        print('----------------------------------')
        ...
        MDM_analysis = market_dynamics_modeling_analysis.MarketDynamicsModelingAnalysis(process_datafile_path,
                                                                                        self.key_indicator)
        ...
        return os.path.abspath(
            process_datafile_path), market_dynamic_modeling_visualization_paths, market_dynamic_modeling_analysis_paths
